#!/usr/bin/env bash
# SFT OpenSubtitles GPT-2-small 20M seeds (0 / 42 / 123) on the same Raven
# train/val JSONL used for BabyLM / CHILDES / TD. Records run_ids to
# babylm_finetune/outputs/opensubtitles_sft_manifest.json
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"

MANIFEST="$ROOT/babylm_finetune/outputs/opensubtitles_sft_manifest.json"
CFG="babylm_finetune/configs/train/sft_opensubtitles.yaml"
...
run_one() {
  local seed="$1"
  local model="os-seed${seed}"
  local run_name="all_types"
  echo ""
  echo "=== OpenSubtitles SFT seed=${seed} model=${model} ==="
  local log
  log="$(mktemp)"
