#!/usr/bin/env bash
# SFT all CHILDES ladder budgets (seed42: 1M / 5M / 12M / 24M) on the same
# Raven train/val JSONL used for BabyLM. Records run_ids to
# babylm_finetune/outputs/childes_sft_manifest.json
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"

MANIFEST="$ROOT/babylm_finetune/outputs/childes_sft_manifest.json"
CFG="babylm_finetune/configs/train/sft_childes.yaml"
TRAIN_FULL="babylm_finetune/data/sft/train.jsonl"
BUDGETS=(1M 5M 12M 24M)
...
if not path.exists():
    path.write_text(
        json.dumps(
            {
                "runs": [],
                "ladder_repo": "mcxfrank/childes-gpt2-ladder",
                "ladder_seed": 42,
                "budgets": ["1M", "5M", "12M", "24M"],
...
run_one() {
  local budget="$1"
  local model="childes-${budget}"
  local run_name="all_types"
  echo ""
  echo "=== CHILDES SFT budget=${budget} model=${model} ==="
  local log
  log="$(mktemp)"
