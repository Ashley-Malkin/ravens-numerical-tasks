#!/usr/bin/env python3
"""Put Wiki GPT-2-small 20M finals onto Modal Volume ``ravens-wiki-base``.

Uploads local folders under ``wiki-models/`` (default)::

    modal run babylm_finetune/scripts/upload_wiki_base_volume.py
    modal run babylm_finetune/scripts/upload_wiki_base_volume.py --seeds 42
    modal run babylm_finetune/scripts/upload_wiki_base_volume.py \\
      --from-local /path/to/wiki-models
"""
...
from ravens_numerical.models.registry import (  # noqa: E402
    WIKI_BASE_VOLUME_MOUNT,
    WIKI_BASE_VOLUME_NAME,
    WIKI_FOLDER_BY_SEED,
    WIKI_LOCAL_ROOT,
    WIKI_SEEDS,
...
app = modal.App("ravens-wiki-base-upload")

wiki_base_volume = modal.Volume.from_name(WIKI_BASE_VOLUME_NAME, create_if_missing=True)
...
def _parse_seeds(raw: str | None) -> tuple[int, ...]:
    ...
    for part in raw.split(","):
        ...
        if not s:
            continue
        seed = int(s)
        if seed not in WIKI_FOLDER_BY_SEED:
...
def _upload_local_to_volume(local_root: Path, seeds: list[int]) -> dict:
    ...
    print(
        f"Uploading from {local_root} → Volume {WIKI_BASE_VOLUME_NAME!r}",
        flush=True,
    )
    with wiki_base_volume.batch_upload(force=True) as batch:
        for seed in seeds:
            folder = wiki_folder_name(seed)
            src = _resolve_local_checkpoint(local_root, folder)
            if src is None:
                raise FileNotFoundError(
                    f"Missing {folder!r} under {local_root}."
                )
            assert_hf_checkpoint_dir(src, label="Wiki checkpoint")
            remote = f"/{folder}"
            print(f"  {src} → {remote}", flush=True)
            batch.put_directory(str(src), remote)
            summary[str(seed)] = f"{WIKI_BASE_VOLUME_MOUNT}/{folder}"

    manifest = {
        "volume": WIKI_BASE_VOLUME_NAME,
        "mount": WIKI_BASE_VOLUME_MOUNT,
        "source": "local",
...
def main(
    ...
    """Upload Wiki finals onto Volume ``ravens-wiki-base``.

    ``seeds`` — comma-separated list (default: 0,42,123).
    ``from_local`` — checkpoint parent (default: repo ``wiki-models/``).
    """
    parsed = list(_parse_seeds(seeds))
    local_root = Path(from_local).expanduser() if from_local else WIKI_LOCAL_ROOT
    result = _upload_local_to_volume(local_root, parsed)

    print("\n=== Wiki base upload complete ===", flush=True)
    for seed, path in sorted(result.get("paths", {}).items(), key=lambda x: int(x[0])):
        print(f"  seed {seed}: {path}", flush=True)
    print(
        f"Models ready at {WIKI_BASE_VOLUME_MOUNT}/<folder> for SFT/eval.",
        flush=True,
    )
