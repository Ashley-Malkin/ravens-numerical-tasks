...
from ravens_numerical.paths import REPO_ROOT, RUNS_DIR, TASKS_JSON
...
class Config:
    ...
    """Path to ``tasks.json`` for ``ravens_numerical``. Defaults to ``data/tasks.json``."""
...
def _instantiate_task(task_name: TaskName, cfg: Config) -> Task:
    if task_name == "ravens_numerical":
        ...
        tasks_json = cfg.ravens_tasks_json or TASKS_JSON
