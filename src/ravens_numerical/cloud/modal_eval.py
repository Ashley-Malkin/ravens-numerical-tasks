...
saves per-trial JSON under ``baby-reasoning/results/`` for audit):
...
def _run_baby_reasoning_cli(
    ...
    results_root = Path(CONTAINER_RUNS_DIR)
    instruction_mode = (
...
def main(
    ...
    Per-trial JSON is saved locally under ``baby-reasoning/results/`` (same layout
    as in-container paths) for downstream audit via ``audit_instruction_results.py``.
    """
