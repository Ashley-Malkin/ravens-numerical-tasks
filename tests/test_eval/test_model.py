...
from ravens_numerical.eval.backends import OllamaBackend, VLLMBackend
...
@pytest.fixture
def ollama_backend():
    return OllamaBackend(model="qwen3:8b", base_url="http://localhost:11434")


@resp.activate
def test_ollama_generate_returns_response_field(ollama_backend):
    resp.add(
        resp.POST,
        OLLAMA_URL,
    ...
    result = ollama_backend.generate("prompt")
    assert result.text == "B"
    assert result.token_logprobs is None


@resp.activate
def test_ollama_generate_falls_back_to_thinking(ollama_backend):
    resp.add(
        resp.POST,
        OLLAMA_URL,
        json={"response": "", "thinking": "C", "done": True},
        status=200,
    )
    result = ollama_backend.generate("prompt")
...
def test_ollama_score_completion_returns_none(ollama_backend):
    assert ollama_backend.score_completion("p", "c") is None
